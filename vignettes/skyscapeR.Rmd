...
If you rather install the latest development versions(often unstable or untested) you can do so directly from the GitHub repository. Firstly, ensure you have package _devtools_ installed, by running:

```
install.packages('devtools')
```

Then type the following to download and install _skyscapeR_:
```
devtools::install_github('f-silva-archaeo/skyscapeR')
```
At the end, you shoudl see the same successful completion line.
